...
const commonCssIcon = (w = 22, h = 22) => {
   return ".icon {  display: inline-block; width: "+w+"px; height: "+h+"px; border-radius: 50%;  transform: rotate(45deg); }"+
      ".icon::before, .icon::after { position: absolute; content: ''; background-color: #fff; }";
};

export const successIcon = (w = 22, h = 22, includeMain = true) => {
    return includeMain ? commonCssIcon(w, h) : '' + '.icon.icon-success { background: green; } '+
...
export const warnIcon = (w = 22, h = 22, includeMain = true) => {
    return includeMain ? commonCssIcon(w, h) : '' +
