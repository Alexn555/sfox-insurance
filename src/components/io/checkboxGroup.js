...
class CheckboxInputGroup extends HTMLElement {
  constructor() {
    super();
  ...
  initCheboxes() {
    if (this.list && this.list.length > 0) {
      for (let i = 0, l = this.list.length; i < l; i++) {
        ...
        el.onchange = (() => {
          CustomEventService.send(`${CustomEvents.interaction.checkboxChange}-${this.name}`, el.value);
        }); 
      }
    } else {
      LoggerService.warn(`CheckboxGroup with id ${this.id} does not have options`);
    }
  }
  ...
  showList() {
    ...
    this.list.forEach((item, index) => {
      html += `
        <div>
          <input type="checkbox" id="${this.id}-${index}" name="${this.name}" value="${item.val}" />
...
if ("customElements" in window) {
  customElements.define("checkbox-input-group", CheckboxInputGroup);
}
